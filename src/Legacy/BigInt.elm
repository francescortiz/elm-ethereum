module Legacy.BigInt exposing
    ( BigInt
    , fromInt, fromString, toString, toHexString
    , add, sub, mul, div, mod, divmod, pow
    , abs, negate
...
mkBigInt s ((Magnitude digits_) as mag) =
    if List.isEmpty digits_ then
        Zer
...
mkBigIntNotNormalised s digits_ =
    BigIntNotNormalised s (MagnitudeNotNormalised digits_)
...
revMagnitudeToString (Magnitude digits_) =
    case List.reverse digits_ of
        [] ->
            "0"
...
normalise (BigIntNotNormalised s digits_) =
    let
        (Magnitude normalisedMag) =
            normaliseMagnitude digits_
    in
...
toPositiveSign bigInt =
    case bigInt of
        ...
        Neg (Magnitude digits_) ->
            mkBigIntNotNormalised Positive (reverseMagnitude digits_)

        Pos (Magnitude digits_) ->
            mkBigIntNotNormalised Positive digits_


isNegativeMagnitude : List Int -> Bool
isNegativeMagnitude digits_ =
    case List.Extra.last digits_ of
        Nothing ->
            False
